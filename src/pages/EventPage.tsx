...
/* ═══════════════════════════ FEATURED EVENT HERO ═══════════════════════════ */
function FeaturedEventHero({ event }: { event: EventItem }) {
  return (
    <div className="relative rounded-2xl overflow-hidden mb-14 group featured-glow cursor-pointer">
      {/* Background image */}
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-end md:items-center gap-6 p-8 md:p-12 min-h-[320px] md:min-h-[360px]">
        <div className="flex-1 max-w-2xl">
          ...
          <div className="flex gap-3">
            ...
            <button className="px-6 py-3 bg-white/10 hover:bg-white/20 text-white font-semibold rounded-xl text-[13px] border border-white/15 transition-all duration-300 backdrop-blur-md">
              Learn More
            </button>
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="relative h-44 overflow-hidden">
        ...
        {/* Hover overlay */}
        <div className="absolute inset-0 bg-[#0A2A4A]/60 opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-center justify-center">
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="flex gap-2 mt-auto pt-3 border-t border-[#E1E5EA]/50">
          {event.status !== 'completed' ? (
            <button className="btn-ripple group/btn flex-1 bg-[#0A2A4A] hover:bg-[#00629B] text-white font-semibold py-2.5 px-4 rounded-xl text-[12.5px] transition-all duration-300 flex items-center justify-center gap-2 hover:shadow-blue">
            ...
            <button className="flex-1 bg-[#F5F7FA] text-[#999] font-semibold py-2.5 px-4 rounded-xl text-[12.5px] cursor-not-allowed border border-[#E1E5EA]/50">
              Event Ended
            </button>
          )}
          <button className="px-4 py-2.5 rounded-xl border border-[#E1E5EA]/80 text-[#00629B] font-semibold text-[12.5px] hover:bg-[#00629B]/[0.05] hover:border-[#00629B]/25 transition-all duration-300">
            Details
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#F5F7FA] text-[#333] font-sans antialiased relative">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 md:px-10 lg:px-14 py-12 md:py-16">
        ...
        <div className="text-center mb-10 animate-fade-in">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#00629B]/[0.06] border border-[#00629B]/10 mb-5">
            <span className="w-1.5 h-1.5 rounded-full bg-[#FFC72C]" />
            <span className="text-[11.5px] font-bold uppercase tracking-[0.18em] text-[#00629B]">
              What's happening
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-1 p-1 bg-white/70 backdrop-blur-md rounded-xl border border-[#E1E5EA]/70 shadow-sm">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                id={`tab-${tab.id}`}
                className={`relative px-4 py-2 rounded-lg text-[12.5px] font-semibold transition-all duration-300 flex items-center gap-1.5 ${
                  activeTab === tab.id
              ...
              >
                ...
                <span
                  className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ml-0.5 ${
                    activeTab === tab.id ? 'bg-white/15 text-white/70' : 'bg-[#E1E5EA]/50 text-[#aaa]'
                  }`}
                >
                  {getTabCount(tab.id)}
                </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6">
            {(activeTab === 'all' && searchQuery === '' ? cardEvents : filteredEvents).map((event, idx) => (
              <div
                ...
                data-id={event.id}
              >
                <EventCard event={event} visible={visible.has(event.id)} delay={idx * 0.06} />
              </div>
            ))}
          </div>
        ) : (
